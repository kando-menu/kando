...
export class KandoApp {
  ...
  private migrateSettings() {
    ...
    ipcMain.handle('settings-window.export-menu-to-json', async (event, menuIndex) => {
      const menu_name = this.menuSettings.get('menus')[menuIndex]['root']['name']
      const { canceled, filePath } = await dialog.showSaveDialog({
        defaultPath: `${menu_name}.json`,
        filters: [{ name: 'JSON File', extensions: ['json'] }],
      });
    ...
    ipcMain.handle('settings-window.import-menu-from-json', async () => {
      ...
      try {
        ...
        const currentMenus = structuredClone(this.menuSettings.get('menus') || []) as IMenu[];
