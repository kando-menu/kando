 ...
 */
export function getParentAngle(item: ISettingsMenuItem) {
...
export function computeDropTarget(
  centerItem: ISettingsMenuItem,
  dragAngle: number,
  dragIndex?: number
): {
  dropTarget: ISettingsMenuItem;
  dropIndex: number;
} {
  ...
  for (let i = 0; i < candidates.length; i++) {
    const child = candidates[i] as ISettingsMenuItem;
 ...
 */
export function createCenterDiv(item: ISettingsMenuItem) {
 ...
 */
export function createChildDiv(item: ISettingsMenuItem) {
  const div = document.createElement('div');
