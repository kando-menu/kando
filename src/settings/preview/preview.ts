...
import { ISettingsMenuItem } from '../common/settings-menu-item';
...
/**
 * This class is responsible for displaying the menu preview of the settings. It supports
...
export class Preview extends EventEmitter {
   ...
   */
  private selectionChain: Array<ISettingsMenuItem> = [];

  /**
   * The menu item which has been selected last time. This item has a special style in the
   * preview and its properties are drawn in the property settings on the right.
   */
  private activeItem?: ISettingsMenuItem = null;
  ...
  /**
   * This method shows the menu preview. This is used when the toolbar of the settings is
   * collapsed.
   */
  public show() {
  ...
  /**
   * This method hides the menu preview. This is used when the toolbar of the settings is
   * expanded.
   */
  public hide() {
   ...
   * initially from the settings for the root menu. If no menu is given, the menu preview
   * will be cleared.
   */
   ...
   * called when the user changes the name or icon in the properties settings.
   */
   ...
   */
  private getCenterItem(): ISettingsMenuItem {
   ...
   */
  private getParentItem(): ISettingsMenuItem | null {
  ...
  private initDragAndDrop() {
    ...
    const getDragIndex = (item: ISettingsMenuItem) => {
  ...
  private drawMenu(transitionAngle?: number) {
    ...
    centerItem.children?.forEach((child) => {
      this.drawItem(child as ISettingsMenuItem, container);
      this.makeDraggable(child as ISettingsMenuItem);
    });
   ...
   */
  private drawItem(item: ISettingsMenuItem, container: HTMLElement) {
   ...
   */
  private makeDraggable(item: ISettingsMenuItem) {
   ...
   */
  private updateChildPosition(child: ISettingsMenuItem) {
    ...
    if (child.type === 'submenu') {
      ...
      child.children.forEach((grandChild, i) => {
        (grandChildContainer.childNodes[i] as HTMLElement).style.setProperty(
          '--rotation',
          (grandChild as ISettingsMenuItem).computedAngle + 'deg'
        );
      });
    }
  }
   ...
   */
  private selectItem(item: ISettingsMenuItem) {
   ...
   */
  private removeItem(item: ISettingsMenuItem) {
    // Remove the child.
    const centerItem = this.getCenterItem();
    const index = centerItem.children.indexOf(item);
    centerItem.children.splice(index, 1);
   ...
   */
  private computeItemAnglesRecursively(
    ...
    for (let i = 0; i < items.length; ++i) {
      const child = items[i] as ISettingsMenuItem;
