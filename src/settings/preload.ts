...
contextBridge.exposeInMainWorld('api', {
  ...
  showMenu: function (
    ...
    ipcRenderer.on('show-menu', (event, root, menuOptions, settingsOptions) =>
      callback(root, menuOptions, settingsOptions)
    );
  },

  /**
   * This will be called by the host process when the user should be shown the settings.
   *
   * @param callback This callback will be called when the settings should be shown.
   */
  showSettings: function (callback: (settingsOptions: IShowSettingsOptions) => void) {
    ipcRenderer.on('show-settings', (event, settingsOptions) =>
      callback(settingsOptions)
    );
  },

  /**
   * This will be called by the host process when the settings should be hidden. This
   * happens for instance when the user clicks on an external link.
   *
   * @param callback This callback will be called when the settings should be hidden.
   */
  hideSettings: function (callback: () => void) {
    ipcRenderer.on('hide-settings', callback);
  },
