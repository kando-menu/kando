...
export class Toolbar extends EventEmitter {
  ...
  /**
   * This method is called initially when the settings is opened. It is used to set the
   * menus and the templates content.
   *
   * @param menuSettings The current settings.
   ...
   * called by the settings whenever the user changed a property of the currently edited
  ...
  private loadContent() {
    ...
    this.container.id = 'kando-settings-toolbar-area';
  ...
  /**
   * Some of the tabs should cover the entire settings. This method wires up the
  ...
  private initTabs() {
    ...
    for (const tab of tabs) {
      ...
      element.addEventListener('shown.bs.tab', () => {
        if (tab.large) {
          document.getElementById('kando-settings-toolbar').classList.add('large');
          this.emit('expand');
        } else {
          document.getElementById('kando-settings-toolbar').classList.remove('large');
          this.emit('collapse');
        }
      });
    }
