...
export class Menu extends (EventEmitter as new () => TypedEventEmitter<MenuEvents>) {
    ...
    if (
      ...
      this.centerText.show(
        ...
        this.settings.underlineQuickSelectKey
          ? newHoveredItem.renderData.quickSelectKeySymbol
          : null
      );
    }
  ...
  private createRenderData(rootItem: RenderedMenuItem, rootContainer: HTMLElement) {
    ...
    while (queue.length > 0) {
      ...
      const quickSelectKeySymbol = MenuTheme.getQuickSelectKeySymbol(item, index);
      ...
      item.renderData = {
        ...
        position: { x: 0, y: 0 },
        computedAngle: angle,
        nodeDiv: this.theme.createItem(item, this.settings, quickSelectKeySymbol),
        connectorDiv: null,
